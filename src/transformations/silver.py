"""Spark Silver transformation with schema, null handling, deduplication and derived features."""
...
SCHEMA = T.StructType([
    ...
    T.StructField("readmitted_30_days", T.IntegerType(), False),
])
...
def transform(df):
    return (
        ...
        .withColumn("risk_score", F.round(
            ...
            + F.when(F.col("age") >= 65, 0.15).otherwise(0), 3))
    )
...
def main():
    ...
    df = spark.read.schema(SCHEMA).parquet(args.input)
    transform(df).write.mode("overwrite").parquet(args.output)
    spark.stop()
