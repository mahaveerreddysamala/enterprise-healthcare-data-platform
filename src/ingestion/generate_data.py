...
def generate_chunk(start: int, rows: int, seed: int) -> pd.DataFrame:
    ...
    cost = np.round(np.maximum(100, 600 + age * 28 + chronic * 1800 + emergency * 4200 + rng.normal(0, 900, size=rows)), 2)
    p_readmit = np.clip(0.04 + 0.08 * chronic + 0.05 * emergency + 0.01 * los, 0, 0.8)
    event_date = pd.Timestamp("2023-01-01") + pd.to_timedelta(rng.integers(0, 1095, size=rows), unit="D")
    result = pd.DataFrame({
        ...
        "payer_type": rng.choice(["commercial", "medicare", "medicaid", "self_pay"], size=rows, p=[0.45, 0.30, 0.20, 0.05]),
    })
